...
output "cognito_token_command" {
  value = format("aws cognito-idp admin-initiate-auth --user-pool-id %s --client-id %s --auth-flow ADMIN_NO_SRP_AUTH --auth-parameters USERNAME=sumeet.n, PASSWORD=Test@123", aws_cognito_user_pool.pool.id, aws_cognito_user_pool_client.client.id)
